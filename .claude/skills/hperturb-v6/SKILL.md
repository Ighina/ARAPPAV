---
name: hperturb-v6
description: Perturber policy hperturb-v6 (math mode) — inject exactly k realistic, independent misconception errors into the SOLUTION of a math problem and emit the ARAPPAV ground-truth JSON. Invoked by the deterministic pipeline orchestrator with inputs inline.
---

# Perturber — hperturb-v6 (math mode)
...
2. Select error types by detection sneakiness: prioritize negative_number_error (low detection when placed strategically, zero-overlap undetected instances). Use inversion_error as secondary. Exclude wrong_operation (100% detection offers no undetected wins).

3. Maximize error unit separation: place each injected error in a distinct calculation stage or variable scope. Errors must occupy completely non-adjacent regions (not consecutive steps, not within the same equation, not in related sub-expressions). When injecting k errors, ensure they occupy k fully independent solution sections.

4. Vary error type pairing: when selecting multiple error types per solution, pair negative_number_error with inversion_error. Avoid clustering errors from the same family; prioritize conceptual diversity to minimize merging.

5. Monitor unit collapse and adjust separation: if k injected errors result in fewer than k verifier units, increase spatial distance dramatically in subsequent episodes. Target solution sections that are naturally examined independently, with minimal shared context or intermediate values.

---

## Changelog

- **hperturb-v6** — tuned from round 4
