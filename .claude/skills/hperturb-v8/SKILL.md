---
name: hperturb-v8
description: Perturber policy hperturb-v8 (math mode) — inject exactly k realistic, independent misconception errors into the SOLUTION of a math problem and emit the ARAPPAV ground-truth JSON. Invoked by the deterministic pipeline orchestrator with inputs inline.
---

# Perturber — hperturb-v8 (math mode)
...
2. Prioritize detection evasion: favor wrong_operation (80% evasion rate in recent trials) and duplication_error (100% evasion rate) as primary targets. Inversion_error is permissible (~29% evasion). Strongly deprioritize additive_thinking due to consistent high detection (~88%). Exclude operand_swap, inverse_operation_error, geometry_definition, whole_number_bias, variable_misconception, negative_number_error, adding_across, wrong_sequence_term, and decimal_magnitude—all show detection rates at or near 100%.

3. Respect solution structure: not all solutions offer k genuinely independent error sites. Before committing to k errors, verify each can occupy a separate calculation branch without sharing intermediate values or appearing in dependent sub-expressions. If the solution supports fewer than k independent sites, inject only for those present rather than accepting unit collapse.

4. Default to conservative injection: inject k ≤ 2 errors unless the solution structure explicitly supports k ≥ 3 with high confidence. A single well-placed stealth error outperforms multiple merged errors that collapse into fewer verifier units.

5. Restrict combinations to proven low-detection pairings: when using multiple errors, pair only from {wrong_operation, duplication_error, inversion_error}. Do not combine any excluded high-detection type with others.

---

## Changelog

- **hperturb-v8** — tuned from round 6
