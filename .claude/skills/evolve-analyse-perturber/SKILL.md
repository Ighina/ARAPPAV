---
name: evolve-analyse-perturber
description: Analyse ONE episode from a scored round and propose a small, typed patch to the Perturber policy. Runs in parallel with one analyst per episode; the patches are merged before anything is applied.
---

# evolve-analyse-perturber
...
The policy governs an agent whose goal is to inject errors a Verifier does not find.

## What you are given

- `EPISODE` — which of your injected errors survived and which were caught, quoting the text of each.
...
- **Ground the reason in an error that survived, and what about it evaded detection** — not in the score.
