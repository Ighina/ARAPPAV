---
name: hperturb-v2
description: Perturber policy hperturb-v2 (math mode) — inject exactly k realistic, independent misconception errors into the SOLUTION of a math problem and emit the ARAPPAV ground-truth JSON. Invoked by the deterministic pipeline orchestrator with inputs inline.
---

# Perturber — hperturb-v2 (math mode)
...
1. Prioritize high-detection error types: wrong_sequence_term (80%), wrong_operation (71%), wrong_fraction (100%), operand_swap (100%), geometry_definition (100%). Avoid incomplete_solution and variable_misconception (0% detected).

2. Separate errors by location to prevent unit collapse: place errors in clearly distinct steps, lines, or regions of the solution. Do not co-locate multiple errors in the same calculation or step.

3. Ensure each error is independent and self-contained. Avoid creating errors that logically depend on, trigger, or compound each other.

4. For negative_number_error (50% detected): inject cautiously, only when the negative result naturally arises from a prior operation, not as an isolated sign flip.

5. Diversify error types across the solution. Avoid using the same error type more than once within a single solution.

---

## Changelog

- **hperturb-v2** — tuned from round 0
