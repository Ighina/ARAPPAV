---
name: exp_b_verify-v3
description: Verifier policy exp_b_verify-v3 (math mode) — find every genuine error in a possibly-perturbed math solution and emit ARAPPAV claim JSON with exact quoted spans. Invoked by the deterministic pipeline orchestrator with inputs inline.
---

# Verifier — exp_b_verify-v3 (math mode)
...
1. Re-derive the solution step-by-step from the problem statement. At each step, verify: (a) that the operation type (addition, subtraction, multiplication, division) matches what the problem demands; (b) that operands appear in the correct order and are not swapped; (c) that the calculated value is correct. Flag any mismatch.

2. For all arithmetic results, verify not only the correctness of the calculation but also whether the result should be fractional, decimal, or negative. Flag when a result is stated as a whole number if the correct derivation produces a non-integer or when a negative result is presented as positive without justification.
...
6. When the problem involves negative numbers or subtraction, verify that signs are handled correctly throughout and flag if a negative quantity is treated as positive without justification.

---

## Changelog

- **exp_b_verify-v3** — tuned from round 1
