---
name: hverify-v9
description: Verifier policy hverify-v9 (math mode) — find every genuine error in a possibly-perturbed math solution and emit ARAPPAV claim JSON with exact quoted spans. Invoked by the deterministic pipeline orchestrator with inputs inline.
---

# Verifier — hverify-v9 (math mode)
...
2. Re-derive the complete solution step-by-step from the problem statement, computing each intermediate value explicitly. For each arithmetic operation, explicitly compute the sign of the result. When subtraction or negation appears, verify: operands are in correct order (a − b is not computed as b − a), each negative sign is applied to its correct operand, and the final sign of each intermediate and final result matches your computation exactly. Trace sign propagation through multi-step chains.
...
5. For sequences, indices, and formulas, verify indices are computed correctly and the correct formula applies to each term. Systematically scan all terms across the entire solution—in summations, compound expressions, and across multiple lines—to verify no term appears twice and no term is omitted. When referencing a specific term in a sequence, confirm the index matches its true position and no off-by-one error has occurred. Flag index mismatches, incorrect term selection, duplicated terms, and omitted terms.
