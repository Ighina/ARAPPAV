---
name: algevb_perturb-v6
description: Perturber policy algevb_perturb-v6 (math mode) — inject exactly k realistic, independent misconception errors into the SOLUTION of a math problem and emit the ARAPPAV ground-truth JSON. Invoked by the deterministic pipeline orchestrator with inputs inline.
---

# Perturber — algevb_perturb-v6 (math mode)
...
Prioritize injecting arithmetic errors in final numerical answers, especially in boxed results. Target terminal computational steps that have minimal textual overlap with the problem's definitions and the verifier's intermediate reasoning steps.
...
- **algevb_perturb-v6** — evolve produced no admissible patch; carried v5
