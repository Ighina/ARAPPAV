---
name: exp_b_perturb-v3
description: Perturber policy exp_b_perturb-v3 (math mode) — inject exactly k realistic, independent misconception errors into the SOLUTION of a math problem and emit the ARAPPAV ground-truth JSON. Invoked by the deterministic pipeline orchestrator with inputs inline.
---

# Perturber — exp_b_perturb-v3 (math mode)
...
1. Inject and declare exactly three distinct error units per problem. Each unit must occupy a separate solution component (different line, operation, or coefficient). Write span-specific declarations (e.g., "wrong_operation in the recurrence relation", "whole_number_bias in the second coefficient") to ensure units remain distinct at detection time and resist collapse.

2. Across the three units, select error types from at least two categories: operational (wrong_operation, operand_swap), progression-based (wrong_sequence_term, incomplete_solution), and numerical (whole_number_bias). Do not repeat a type within one problem. Spread types deliberately to diversify declared units.

3. wrong_operation has 40% detection and generates wins when undetected. Deploy as one or two units per problem in intermediate steps, coefficients, or term indices—avoid sole-error placement. Low detectability makes this type valuable.

4. whole_number_bias appears often; inject when rounding, fractional coefficients, or integer approximation naturally arise. Declare its exact location (e.g., "in the recurrence step" or "in the scaling factor") to keep it separate from other units.

5. Reserve variable_misconception, additive_thinking, and operand_swap for problems where scope ambiguity or operation choice is genuinely tempting. Do not inject simply to fill a category. These types are less frequent and overfitting to any single problem type weakens the policy.

---

## Changelog

- **exp_b_perturb-v3** — tuned from round 1
