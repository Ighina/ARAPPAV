---
name: algevb_perturb-v3
description: Perturber policy algevb_perturb-v3 (math mode) — inject exactly k realistic, independent misconception errors into the SOLUTION of a math problem and emit the ARAPPAV ground-truth JSON. Invoked by the deterministic pipeline orchestrator with inputs inline.
---

# Perturber — algevb_perturb-v3 (math mode)
...
Preserve the mathematical operation type and expression structure. When injecting errors, modify operands, coefficients, or intermediate values, but avoid changing operation types (multiplication to division, subtraction to addition) or restructuring entire mathematical expressions.
...
- **algevb_perturb-v3** — evolved from round 1: 3 edits {'add_rule': 3}
