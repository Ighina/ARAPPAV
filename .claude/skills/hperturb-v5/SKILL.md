---
name: hperturb-v5
description: Perturber policy hperturb-v5 (math mode) — inject exactly k realistic, independent misconception errors into the SOLUTION of a math problem and emit the ARAPPAV ground-truth JSON. Invoked by the deterministic pipeline orchestrator with inputs inline.
---

# Perturber — hperturb-v5 (math mode)
...
1. Enforce strict type diversity: use each error type at most once per solution. Do not repeat types even when injecting multiple errors.

2. Select error types by round 3 observed detection rate: prioritize inversion_error (83%), wrong_operation (75%), negative_number_error (75%). Exclude types with 0% detection in round 3 (incomplete_solution, additive_thinking).

3. Maximize error unit separation: place each injected error in a distinct calculation stage or variable scope. Errors must occupy completely non-adjacent regions (not consecutive steps, not within the same equation, not in related sub-expressions). When injecting k errors, ensure they occupy k fully independent solution sections—e.g., setup, main calculation, verification.

4. Vary error type pairing: when selecting multiple error types per solution, pair errors from different calculation families. Avoid clustering arithmetic-family errors together; instead combine wrong_operation with inversion_error or negative_number_error. Diversity across error families reduces the likelihood that separate errors merge into a single verifier unit.

5. Monitor unit collapse and adjust separation: track whether injected k errors result in fewer than k verifier units. If collapse persists across multiple episodes, increase the distance between subsequent error injections—use structurally independent problem sections, separate calculation stages, or geographically distant solution regions.

---

## Changelog

- **hperturb-v5** — tuned from round 3
