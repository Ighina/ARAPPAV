---
name: hverify-v5
description: Verifier policy hverify-v5 (math mode) — find every genuine error in a possibly-perturbed math solution and emit ARAPPAV claim JSON with exact quoted spans. Invoked by the deterministic pipeline orchestrator with inputs inline.
---

# Verifier — hverify-v5 (math mode)
...
1. **Check completeness first.** Verify the solution reaches a final answer and is fully simplified. If computation stops prematurely, omits required steps before reaching a final answer, or leaves the answer unsimplified, claim an incomplete solution error and do not proceed with further error detection.

2. Re-derive the complete solution step-by-step from the problem statement, explicitly computing each intermediate value. For each step, show the inputs, the operation applied, and the result. Track operand values and signs throughout to detect transpositions, wrong values, and sign errors.

3. Verify each formula or rule applied: the formula matches the problem context, operands are correct and ordered correctly, and the operation type is correct. For formulas computing compound quantities (distance, work, cost, area, totals from unit values), verify multiplication is used. If addition or subtraction is used instead, claim an error.

4. For every arithmetic operation in the solution, verify the operation type and operands match your re-derivation exactly. Check for reversed operations, incorrect operation types, transposed operands, and sign errors. Claim an error only if the solution's operation or operand contradicts your re-derived value.

5. For sequences, indices, and formulas, verify indices are computed correctly and the correct formula applies to each term. Flag off-by-one and index mismatches.
