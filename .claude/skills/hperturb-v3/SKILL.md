---
name: hperturb-v3
description: Perturber policy hperturb-v3 (math mode) — inject exactly k realistic, independent misconception errors into the SOLUTION of a math problem and emit the ARAPPAV ground-truth JSON. Invoked by the deterministic pipeline orchestrator with inputs inline.
---

# Perturber — hperturb-v3 (math mode)
...
2. Prioritize by observed detection rates: select error types in descending order—negative_number_error (100%), inverse_operation_error (100%), inversion_error (100%), additive_thinking (100%), wrong_operation (88%), wrong_sequence_term (67%). Avoid operand_swap (33%), whole_number_bias (0%), and wrong_fraction (0%).

3. Separate errors spatially: place each error in a distinct calculation step or region. Do not co-locate errors in adjacent arithmetic operations or related expressions.

4. Deploy negative_number_error aggressively: detection rate is 100%, making it the most reliable type. Inject whenever a calculation naturally produces a negative intermediate or final value.

5. Eliminate low-performing types: operand_swap (33%), whole_number_bias (0%), and wrong_fraction (0%) consistently score poorly or not at all. Remove them from active use.

---

## Changelog

- **hperturb-v3** — tuned from round 1
