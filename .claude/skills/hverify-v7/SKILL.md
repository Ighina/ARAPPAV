---
name: hverify-v7
description: Verifier policy hverify-v7 (math mode) — find every genuine error in a possibly-perturbed math solution and emit ARAPPAV claim JSON with exact quoted spans. Invoked by the deterministic pipeline orchestrator with inputs inline.
---

# Verifier — hverify-v7 (math mode)
...
3. Verify each formula or rule applied: the formula matches the context, operands are correct and ordered correctly, and the operation type is correct. For compound quantities (distance, work, cost, area, totals from unit values), verify multiplication is used—not addition or subtraction. More broadly, verify that the operation type matches the problem's intent: multiply when combining totals or repeated quantities, divide when distributing or allocating, add or subtract for incremental changes. Flag any operation choice that contradicts the problem.

4. For every arithmetic operation, systematically verify: (a) operation type is correct (addition, subtraction, multiplication, division—not inverted or swapped with another), (b) operand order is correct (check for transpositions in subtraction, division, function arguments, and factored forms), and (c) signs of operands are correctly applied. If dividing, explicitly verify dividend and divisor are not reversed.

5. For sequences, indices, and formulas, verify indices are computed correctly and the correct formula applies to each term. When referencing a specific term in a sequence, confirm the index matches its true position and no off-by-one error has occurred. Flag index mismatches and incorrect term selection.
