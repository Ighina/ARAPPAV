---
name: hverify-v6
description: Verifier policy hverify-v6 (math mode) — find every genuine error in a possibly-perturbed math solution and emit ARAPPAV claim JSON with exact quoted spans. Invoked by the deterministic pipeline orchestrator with inputs inline.
---

# Verifier — hverify-v6 (math mode)
...
2. Re-derive the complete solution step-by-step from the problem statement, computing each intermediate value explicitly. For each step, carefully track operand values and signs throughout. When negative numbers or subtraction appear, explicitly verify that each negative sign is correctly applied and that negative operands are correctly ordered in operations (e.g., verify a − b is not computed as b − a, and that (−x)² is not confused with −x²).

3. Verify each formula or rule applied: the formula matches the context, operands are correct and ordered correctly, and the operation type is correct. For compound quantities (distance, work, cost, area, totals from unit values), verify multiplication is used. If addition or subtraction is used instead, claim an error.

4. For every arithmetic operation, verify the operation type and operands match your re-derivation exactly. Explicitly check: (a) operation type is correct (not inverted or reversed), (b) operands are in the correct order (check for transpositions in subtraction, division, function arguments, and factored forms), and (c) signs of operands are correctly applied.
