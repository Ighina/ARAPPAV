---
name: perturb-v2
description: Perturber policy v2 (math mode) — inject exactly k realistic, independent misconception errors into a correct math solution and emit the ARAPPAV ground-truth JSON. Use when acting as the Perturber in a skill self-play rollout, or when asked to perturb a math solution with perturb-v2.
---
...
> when it produces `perturb-v3`. Everything above stays fixed.
...
independent units need `k` such positions. A short solution simply does not have them —
placing the surplus error anyway produces a merge, not a harder episode. When branches
exist, spend one error per branch first: they cannot merge, because nothing links them.
...
**P3 — Hide the error where the Verifier is told not to look.** A competent Verifier is
under instruction to claim the earliest wrong line and to skip values that merely carry
forward an upstream mistake. So the most durable placement is a step that *looks* like the
mechanical consequence of an earlier error but is independently invalid — a rearrangement
whose form is wrong, not just its inputs. The reader sees a number that "follows" and moves
on. Choose one error per episode to sit in this position.

**P4 — Choose the misconception by what survives re-derivation.** A flipped sign, a swapped
pair of operands, an off-by-one index and a corrupted visible constant are all caught by the
first careful re-reading — they are wrong *on their face*. What survives is bookkeeping
inside a rearrangement: an inverse operation applied in the wrong direction, a factorisation
or identity whose shape is invalid while its constants look right, a step that is locally
well-formed and only fails when re-derived symbolically. Prefer those. Spread types across
an episode and across rounds — repeating a trick between rounds costs −5.0.

**P5 — Anchor spans on complete mathematical expressions.** `original_text` and
`injected_text` must be a whole equation or clause, both sides included. Do not straddle
prose and mathematics (a verb plus a left-hand side), and do not stop before the right-hand
side. The matcher aligns a claim to your span by containment or ≥0.5 span IoU; an anchor
that a reader would never quote back verbatim fails both, so a Verifier who found the error
is scored as having missed it. That is a false win, and it is the one span shape to avoid.
...
**P8 — Self-check before writing.** In order: exactly `k` errors; no declared error changes
...
`perturbed_solution`; every span a complete expression; no two errors touching the same
