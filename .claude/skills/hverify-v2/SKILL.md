---
name: hverify-v2
description: Verifier policy hverify-v2 (math mode) — find every genuine error in a possibly-perturbed math solution and emit ARAPPAV claim JSON with exact quoted spans. Invoked by the deterministic pipeline orchestrator with inputs inline.
---

# Verifier — hverify-v2 (math mode)
...
1. Re-derive the complete solution step-by-step from the problem statement, showing all algebraic manipulations, operation applications, and formula selections. Use the correct definitions and rules for the mathematical domain (arithmetic sequences, exponents, fractions, etc.).

2. For each algebraic rule applied in the given solution (exponent distribution, power of a product, fraction simplification, etc.), verify it is applied correctly. Flag incomplete applications (e.g., distributing an exponent to only part of a product) and incorrect rule selections.

3. Check for sign errors and operation errors: compare each operation in the given solution (addition, subtraction, multiplication, division, exponentiation) against the correct operation from your re-derivation. Flag any differences, including missing negation or incorrect sign changes.

4. Check sequence and index calculations: for problems involving sequences, formulas, or indexed terms, verify that indices are computed correctly and that the correct formula (arithmetic, geometric, recursive) is applied. Flag index errors and off-by-one mistakes.

5. Verify variable definitions and substitutions are consistent and correct throughout the solution. Flag cases where a variable is misused, redefined incorrectly, or substituted with the wrong expression.

6. Verify the solution is complete and fully simplified. Flag cases where steps are missing, intermediate results are not computed, or a simplified final form is not reached.

7. For each error identified, quote the exact expression or sub-expression from the given solution that contains the error, using the original mathematical notation. Do not paraphrase or summarize the span; quote exactly as it appears.

8. Create one claim per distinct error. If multiple independent errors appear in the same quoted span, separate them into distinct claims with separate quotes, each explaining its own error.

---

## Changelog

- **hverify-v2** — tuned from round 0
