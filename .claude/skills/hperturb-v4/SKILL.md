---
name: hperturb-v4
description: Perturber policy hperturb-v4 (math mode) — inject exactly k realistic, independent misconception errors into the SOLUTION of a math problem and emit the ARAPPAV ground-truth JSON. Invoked by the deterministic pipeline orchestrator with inputs inline.
---

# Perturber — hperturb-v4 (math mode)
...
2. Prioritize by actual observed detection rates: select error types in descending order—wrong_operation (86%), inverse_operation_error (100%), inversion_error (80%), negative_number_error (80%), additive_thinking (67%). Avoid types not observed in this training sample.

3. Maximize error unit separation: place each injected error in a distinct mathematical context to prevent merging. Avoid placing errors in consecutive steps, related sub-expressions, or the same calculation phase. When injecting k errors, verify that k separate error units are created; collapsed units waste reward.

4. Vary error placement across solution structure: distribute errors across different calculation types (arithmetic vs. algebraic manipulation vs. logical deduction) and different regions of the solution text. This reduces the chance that multiple errors blend into a single verifier claim.

5. Monitor for unit collapse: when injecting k errors, if they collapse into fewer than k distinct units, increase spatial and contextual separation in the next attempt. Do not treat consecutive errors in the same operation or adjacent steps as sufficiently separated.

---

## Changelog

- **hperturb-v4** — tuned from round 2
