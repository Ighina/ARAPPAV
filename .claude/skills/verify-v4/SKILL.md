---
name: verify-v4
description: Verifier policy v4 (math mode) — find every genuine error in a possibly-perturbed math solution and emit ARAPPAV claim JSON with exact quoted spans. Use when acting as the Verifier in a skill self-play rollout, or when asked to verify a math solution with verify-v4.
---

# Verifier — policy v4 (math mode)
...
> when it produces `verify-v5`. Everything above stays fixed.
...
**V3 — Sweep the misconceptions, then audit the model against the problem statement.**
Sign handling across a move between sides; numerator/denominator or dividend/divisor order;
common denominators; additive reasoning where the relation is multiplicative; off-by-one in
sequence indices; probability outside [0,1]; an inverse operation applied in the wrong
direction; an identity or factorisation whose *form* is invalid while its constants look
plausible.

Then do the pass that arithmetic cannot do for you, and give it the same weight as the
re-derivation, because it is where the errors you miss actually live. Go back to the problem
statement and check the solution's model against it line by line: what each symbol denotes,
what a named ratio compares, which set or range a stated condition actually picks out, which
quantity is being minimised or maximised, and whether the thing finally reported is the thing
that was asked for. A wrong *set membership* or a wrong *objective* leaves every subsequent
computation internally flawless, so no amount of re-derivation will surface it — a solution
can be arithmetically perfect and still answer the wrong question. When a step enumerates,
bounds, or selects the objects under discussion, re-read the problem's own wording for that
rule and confirm the enumeration obeys it.
...
**V8 — One claim per distinct error, each with its own quote — including two in one line.**
A claim is a quote plus a reason, and only the quote is scored, so prose describing a mistake
you did not separately anchor counts for nothing. This bites hardest where two errors share a
single expression: the same term misevaluated on both sides of one equation, two coordinates
of one point, two wrong factors inside one radicand. The natural instinct is to quote the
whole line once and explain both faults; that reports one error. Split it — quote each side,
each coordinate, each factor separately, so long as every quote still meets the meaningfulness
floor of three words or a complete expression. Before writing an episode's output, count the
distinct faults your reasoning found and check that you filed that many claims.
...
- **v4** — tuned on round 3 (8 episodes vs `perturb-v3`: F1 0.75, recall 0.688, precision
  0.875 — a genuine regression, and unlike round 2 most of it is real).
  **V8 worked where it was tested cleanly and is now extended.** In ep04 the Perturber planted
  the same sum/product confusion in two separate sentences; V8 caught it, filed two anchored
  claims and scored 3/3. But in ep00, ep01 and ep02 the linked pair sat inside a *single
  expression*, one quote spanned both faults, and only one could match — in each case the
  claim scored 0.900 against the second error and was already consumed by the first. V8 now
  says explicitly to split a shared line into one quote per fault and to count faults against
  claims before writing.
  **V3's definitions pass is rewritten and given equal weight to re-derivation.** ep03 was the
  worst miss of the entire run: two errors that misread the problem's own stopping rule for
  which terms belong to each sequence, leaving every arithmetic line correct. One claim was
  filed on that episode and neither error drew any claim at all. The rule now names set
  membership, bounds, enumeration rules and the objective as things to re-read against the
  problem statement, and states plainly that a solution can be arithmetically perfect and
  still answer the wrong question.
  *Rejected as exploitation:* nothing, and one recorded false positive should be read the
  other way — in ep00 the Verifier correctly identified that the extraction takes a
  non-square factor out of the radical, which is the real mistake in that episode; the ground
  truth had been anchored one step earlier on a regrouping that is a valid equality, so being
  right cost precision. Filed under `scorer_issues`.
