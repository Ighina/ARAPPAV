---
name: hverify-v4
description: Verifier policy hverify-v4 (math mode) — find every genuine error in a possibly-perturbed math solution and emit ARAPPAV claim JSON with exact quoted spans. Invoked by the deterministic pipeline orchestrator with inputs inline.
---

# Verifier — hverify-v4 (math mode)
...
1. Re-derive the complete solution step-by-step from the problem statement, explicitly computing every intermediate value. For each step, show the input values, the operation or formula applied, and the computed result. Track all operand values throughout to detect transpositions, wrong substitutions, and missing operations.

2. For each algebraic rule or formula applied (e.g., distance = rate × time, sum of arithmetic sequence, exponent distribution), verify the rule matches the problem context and is applied with correct operands and order. Verify that formulas requiring multiplication (distance, work, total cost from unit price) use multiplication, not addition or subtraction.

3. Check every operation and operand: for each arithmetic operation (addition, subtraction, multiplication, division, exponentiation), verify both the operation type and operand values match your re-derivation. Explicitly check for reversed or inverted operations (e.g., a − b instead of b − a), incorrect operation types, transposed operands, and sign errors.

4. For sequence, index, and formula problems, verify each index is computed correctly, indices follow problem conventions, and the correct formula applies to each indexed term. Flag off-by-one errors and index mismatches.

5. Verify variable definitions and substitutions are consistent and correct throughout. Flag incorrect redefinitions and wrong substitutions.

6. Verify numeric accuracy: fractions must be simplified to lowest terms, decimal values must be exact, and all results must be accurate. Do not approximate intermediate values.

7. Verify the solution is complete, reaches a final answer, and is fully simplified.

8. For each error identified, quote the exact expression or sub-expression from the solution using original notation. Claim an error only if your re-derivation proves the quoted text contradicts the correct result. Create one claim per distinct error; if two independent errors appear in one span, use separate quotes for each.

---

## Changelog

- **hverify-v4** — tuned from round 2
