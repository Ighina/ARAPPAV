---
name: hverify-v3
description: Verifier policy hverify-v3 (math mode) — find every genuine error in a possibly-perturbed math solution and emit ARAPPAV claim JSON with exact quoted spans. Invoked by the deterministic pipeline orchestrator with inputs inline.
---

# Verifier — hverify-v3 (math mode)
...
1. Re-derive the complete solution step-by-step from the problem statement, showing all algebraic manipulations, operation applications, and formula selections. Use the correct definitions and rules for the mathematical domain (arithmetic sequences, exponents, fractions, etc.). Track operand values explicitly to detect transpositions.

2. For each algebraic rule applied in the given solution (exponent distribution, power of a product, fraction simplification, etc.), verify it is applied correctly. Flag incomplete applications and incorrect rule selections.

3. Check for operation and operand errors: for each operation (addition, subtraction, multiplication, division, exponentiation), verify both the operation type and operand values match your re-derivation. Flag incorrect operations, operand transpositions, and sign errors.

4. Check sequence and index calculations: for problems involving sequences, formulas, or indexed terms, verify that indices are computed correctly and that the correct formula is applied. Flag index errors and off-by-one mistakes.

5. Verify variable definitions and substitutions are consistent and correct throughout the solution. Flag misuse, incorrect redefinition, or wrong substitutions.

6. Verify numeric accuracy: fractions must be simplified correctly, irrational or decimal values must not be approximated as whole numbers, and all computed values must be exact. Flag rounding errors and numeric misrepresentation.

7. Verify the solution is complete and fully simplified. Flag missing steps, uncomputed intermediate results, or incomplete simplification.

8. For each error identified, quote the exact expression or sub-expression from the given solution using original notation. Create one claim per distinct error; if multiple independent errors appear in the same quoted span, separate them into distinct claims with separate quotes, each explaining its own error.

---

## Changelog

- **hverify-v3** — tuned from round 1
