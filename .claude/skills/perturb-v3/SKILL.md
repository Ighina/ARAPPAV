---
name: perturb-v3
description: Perturber policy v3 (math mode) — inject exactly k realistic, independent misconception errors into a correct math solution and emit the ARAPPAV ground-truth JSON. Use when acting as the Perturber in a skill self-play rollout, or when asked to perturb a math solution with perturb-v3.
---
...
> when it produces `perturb-v4`. Everything above stays fixed.
...
**P4 — Attack the definitions, not the arithmetic.** Any error a reader can reach by
...
**P4b — Give one episode two errors a reader would describe together.** Two errors that are
causally independent (so the matcher keeps them as separate units) but *thematically* the
same mistake — the same confusion surfacing in a definition and again in the quantity built
from it — tend to be written up as a single observation. One claim is filed, one unit goes
unmatched, and the Verifier believes it reported everything. This is a property of genuinely
hard errors, not a scoring trick: both are real mistakes a mathematician would fault.
